...
router.post(
  ...
  authMiddleware,
  roleMiddleware('owner'),
  upload.fields([{ name: 'images' }, { name: 'menu' }]),
  async (req, res) => {
    try {
      const {
        name,
      ...
      const amenitiesArray = amenities ? parseArrayField(amenities, 'amenities') : [];
      const spaceArray = space ? parseArrayField(space, 'space') : [];
      const outsideAlcoholArray = outsideAlcohol ? parseArrayField(outsideAlcohol, 'outsideAlcohol') : [];
      const djPolicyArray = djPolicy ? parseArrayField(djPolicy, 'djPolicy') : [];
      ...
      const menuFiles = req.files['menu'] || [];

      // Separate upload for images and menus
      const imageUrls = await uploadFilesToGitHub(imageFiles, 'halls');
      const menuUrls = await uploadFilesToGitHub(menuFiles, 'menus');

      if (imageUrls.length === 0) {
        return res.status(500).json({ msg: 'Failed to upload images to GitHub' });
      }

      // Debug to ensure correct URLs are assigned
      console.log('Uploaded Images:', imageUrls);
      console.log('Uploaded Menus:', menuUrls);

      const hall = new Hall({
        name,
        description,
        imagesUrl: imageUrls, // Image URLs correctly assigned here
        menuURL: menuUrls,       // Menu URLs correctly assigned here
        accommodation: accommodationNumber,
        amenities: amenitiesArray,
        vegPlatePrice: vegPrice,
        nonvegPlatePrice: nonVegPrice,
